    ...
    <div className="min-h-screen w-full flex flex-col md:flex-row relative overflow-hidden">
      ...
      <aside className="hidden md:flex md:w-64 glass-sidebar flex-col shrink-0 p-6 justify-between h-screen sticky top-0">
        ...
        {user ? (
          <button 
            ...
            className="flex items-center gap-3 w-full py-3 px-4 rounded-2xl font-bold text-xs text-white/60 hover:bg-rose-500/15 hover:text-rose-400 transition-all cursor-pointer duration-200"
          >
            <LogOut size={18} />
            <span>Log Out</span>
          </button>
      ...
      <div className="flex-grow flex flex-col min-h-screen">
        ...
        <header className="md:hidden sticky top-0 z-30 w-full flex justify-between items-center py-3.5 px-4 glass-nav glass-shadow">
          <div className="flex items-center gap-3">
            <img src="/hero.png" alt="Healthy Eye Clinic Logo" className="w-9 h-9 object-contain rounded-xl shadow-sm border border-white/40" />
            <h1 className="text-sm font-black text-teal-400 leading-tight">Healthy Eye Clinic</h1>
          </div>
          {user ? (
            <button onClick={handleLogout} className="w-8 h-8 rounded-xl bg-white/10 flex items-center justify-center text-white/60 hover:text-rose-400 hover:bg-rose-500/15 transition-all active:scale-95 cursor-pointer" title="Log Out">
              <LogOut size={16} />
            </button>
          ) : (
            <button onClick={() => setShowLoginModal(true)} className="py-1.5 px-3 rounded-xl bg-teal-500/15 text-teal-400 text-xs font-bold hover:bg-teal-500/25 transition-all active:scale-95 cursor-pointer border border-teal-500/25" title="Sign In">
              Sign In
            </button>
          )}
        </header>
        ...
        <nav className="md:hidden fixed bottom-5 left-1/2 -translate-x-1/2 glass-nav glass-shadow-lg flex justify-around items-center py-2 px-3 z-40 w-[92%] max-w-sm rounded-2xl">
          {[
            ...
            { id: "request", icon: <Calendar size={18} />, label: "Book" },
            { id: "records", icon: <Clock size={18} />, label: "Records" },
        ...
        {showLoginModal && (
          <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-md z-50 flex items-center justify-center p-4">
            <div className="glass-strong rounded-3xl p-8 glass-shadow-lg animate-in fade-in zoom-in-95 duration-300 w-full max-w-sm relative text-center">
              {/* Close button */}
              <button 
                onClick={() => setShowLoginModal(false)}
                className="absolute top-4 right-4 w-7 h-7 rounded-lg bg-white/10 flex items-center justify-center text-white/60 hover:text-white hover:bg-white/20 transition-all cursor-pointer"
              >
                <X size={15} />
              </button>
              ...
              <div className="flex flex-col items-center mb-6">
                <div className="relative mb-4">
                  <div className="absolute inset-0 rounded-2xl bg-teal-500/15 blur-xl scale-125" />
                  <img src="/hero.png" alt="Healthy Eye Clinic Logo" className="relative w-18 h-18 object-contain rounded-2xl shadow-lg border border-white/30" />
                </div>
                <h2 className="text-lg font-extrabold text-white mb-0.5">Please Sign In</h2>
                <p className="text-[10px] font-bold text-teal-400 uppercase tracking-widest leading-none mt-1">To book your appointment</p>
              </div>
              ...
              <button
                ...
                className="w-full py-3.5 px-6 glass border border-white/15 hover:border-teal-500/50 rounded-2xl flex items-center justify-center gap-3 text-white font-bold transition-all duration-300 hover:shadow-lg hover:bg-white/10 active:scale-[0.98] group cursor-pointer"
              >
