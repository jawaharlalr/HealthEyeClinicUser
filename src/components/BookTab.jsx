              ...
              <button
                ...
                className={`py-3 px-2 rounded-xl text-xs font-bold border transition-all cursor-pointer flex flex-col items-center justify-center gap-1 ${
                  isSelected
                    ...
                    : isAvailable
                    ? "bg-white text-primary border-slate-200 hover:bg-primary-light/40 hover:border-primary/30"
                    : "bg-slate-50 text-slate-350 border-slate-100/50 cursor-not-allowed opacity-50 line-through"
                }`}
              >
                <span>{slot}</span>
                <span className={`text-[8px] font-extrabold uppercase tracking-wide px-1.5 py-0.5 rounded ${
                  isSelected
                    ...
                    : "bg-slate-100 text-slate-400 no-underline"
                }`}>
  ...
  return (
    <div className="bg-white/80 backdrop-blur-md border border-slate-200/40 rounded-3xl p-6 md:p-8 shadow-sm animate-in fade-in duration-300 max-w-2xl mx-auto">
      ...
      <form onSubmit={handleBookAppointment} className="space-y-5">
        ...
        >
          {isFormInvalid ? "Fix errors to submit" : "Submit Appointment Request"}
        </button>
      </form>
